...
class Slack extends webhook_1.IncomingWebhook {
    ...
    get fields() {
        ...
        const runner = `${repo_url}/actions/runs/${run_id}`;
        const { number } = context.issue;
        const pullRequest = `${repo_url}/pull/${number}`;
        const fields = [
            {
                short: true,
                title: `repository`,
            ...
            { short: true, title: `runner`, value: `<${runner}|Action Run ${run_id}>` },
            { short: true, title: `Pull Request`, value: `<${pullRequest}|${repo}/pull/${number}>` },
            { short: true, title: `workflow`, value: `<${action_url}|${workflow}>` }
        ];
        core.debug(`run_id : ${run_id}`);
        core.debug(`Debug value GitHub JSON : ${JSON.stringify(github.context, null, 2)}`);
        return fields;
    }
    /**
     * Generate payload
